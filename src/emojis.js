...
function parseCustomEmojis(selector) {
  $.getJSON("https://snippet.host/sgyddt/raw", function (data) {
    $(selector).each(function () {
      var post_body = $(this);
      ...
      data.forEach(function (category) {
        category.emojis.forEach(function (emoji) {
          var regex = new RegExp(":" + emoji.id + ":", "g");
          html_content = html_content.replace(regex, `
          <img class="custom-emoji" draggable="false" src="${emoji.skins[0].src}" alt=":${emoji.id}:" title=":${emoji.id}:">`);
        });
      });
...
$(".post-editor-header-action[title=Spoiler]").after(`<li class="post-editor-header-action emoji-button" title="Emoji Picker"><i class="fa fa-smile-o"></i></li>`);
...
$.getJSON("https://snippet.host/sgyddt/raw", function (custom) {
  ...
  $(picker).appendTo("body").attr("id", "popup");

  const emoji_button = $(".emoji-button")[1];
  const popup = $("#popup")[0];
  ...
  $(document).on("click", ".emoji-button", togglePopup);
  ...
  function togglePopup() {
    if (popper) {
      ...
      $("#popup").show();
      const popup_input = popup.shadowRoot.querySelector("input");
      popup_input.focus({ preventScroll: true });
    }
  }


  // Insert emoji into textarea on select
